...
Workflow carousel images are the exception: they are manually managed under
`public/images/workflow/` and verified by the image contract, not regenerated by
`scripts/optimize-images.js`. Treat that folder as canonical runtime input until
matching source originals are recovered and wired into the optimizer.
