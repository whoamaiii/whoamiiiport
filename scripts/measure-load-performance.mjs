...
const NOTABLE_RESOURCE_PATTERN =
  /assets\/|liquid-perception|textile-corridor|kaaffe-texture-motion-poster|living-floor|mushroom-offering|psych-depth-embed-focus-poster|typekit/;
  ...
  try {
    ...
    const topMetrics = await page.evaluate(() => {
      ...
      const lowerGalleryResourcesBeforeScroll = resources
        .filter((entry) =>
          /\/images\/(?:kaaffe-texture-motion-poster|living-floor|mushroom-offering|psych-depth-embed-focus-poster)-/.test(entry.name),
        )
